...
export default function BackOfficeTreatment() {
  ...
  const [pageSettings, setPageSettings] = useState({
    ...
    treatment_hero_image: 'https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?q=80&w=1600&auto=format&fit=crop',
    treatment_cta_text: 'Explore Treatments',
    treatment_consultation_link: 'https://wa.link/o7f5yk'
  });

  // Categories list
  const [categories, setCategories] = useState<TreatmentCategory[]>(DEFAULT_CATEGORIES);
      ...
      try {
        ...
        if (json.status === 'success' && json.data) {
          ...
          setPageSettings(prev => ({
            ...prev,
            treatment_hero_badge: json.data.treatment_hero_badge || prev.treatment_hero_badge,
            treatment_hero_title: json.data.treatment_hero_title || prev.treatment_hero_title,
            treatment_hero_subtitle: json.data.treatment_hero_subtitle || prev.treatment_hero_subtitle,
            treatment_hero_image: json.data.treatment_hero_image || prev.treatment_hero_image,
            treatment_cta_text: json.data.treatment_cta_text || prev.treatment_cta_text,
            treatment_consultation_link: json.data.treatment_consultation_link || prev.treatment_consultation_link,
          }));

          if (json.data.treatment_categories) {
            try {
              const parsed = JSON.parse(json.data.treatment_categories);
              if (Array.isArray(parsed) && parsed.length > 0) {
                setCategories(parsed);
              }
            } catch (e) {
              console.error('Error parsing treatment_categories', e);
            }
          }
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    const payload: Record<string, string> = {
      ...pageSettings,
      treatment_categories: JSON.stringify(categories)
    };
  ...
  const handleAddService = (catIndex: number) => {
    ...
    const newService: TreatmentItem = {
      name: 'New Service',
    ...
    updatedCat.featuredTreatments = [...(updatedCat.featuredTreatments || []), newService];
    updatedCat.treatmentsCount = `${updatedCat.featuredTreatments.length} Services`;
    
    const newCategories = [...categories];
    newCategories[catIndex] = updatedCat;
    setCategories(newCategories);
    setExpandedServiceIndex(updatedCat.featuredTreatments.length - 1);
  };

  const handleDeleteService = (catIndex: number, serviceIndex: number) => {
    if (confirm('Delete this service?')) {
      const updatedCat = { ...categories[catIndex] };
      updatedCat.featuredTreatments = updatedCat.featuredTreatments.filter((_, idx) => idx !== serviceIndex);
      updatedCat.treatmentsCount = `${updatedCat.featuredTreatments.length} Services`;
      const newCategories = [...categories];
      newCategories[catIndex] = updatedCat;
      setCategories(newCategories);
  ...
  const handleMoveService = (catIndex: number, serviceIndex: number, direction: 'up' | 'down') => {
    const updatedCat = { ...categories[catIndex] };
    const items = [...updatedCat.featuredTreatments];
    ...
    updatedCat.featuredTreatments = items;
    const newCategories = [...categories];
    newCategories[catIndex] = updatedCat;
    setCategories(newCategories);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-brand-beige pb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-serif font-bold text-brand-charcoal">Treatment Page Management</h1>
          </div>
          <p className="text-sm text-brand-charcoal/60">
            Manage treatment categories, services lists, pricing, and hero banner settings.
          </p>
        </div>

        <div className="flex items-center gap-3">
          <Link href="/treatment" target="_blank" className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-brand-forest/30 text-brand-forest text-xs font-semibold hover:bg-brand-forest/5 transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-brand-charcoal/60">{cat.treatmentsCount}</span>
                        </div>
                    ...
                    <div className="p-5 bg-brand-white/40 space-y-6">
                      ...
                      <div className="bg-white rounded-xl border border-brand-beige p-5 space-y-4">
                        <div className="flex items-center justify-between border-b border-brand-beige pb-3">
                          <h4 className="font-serif font-bold text-brand-charcoal text-lg">Services / Protocols ({cat.featuredTreatments?.length || 0})</h4>
                        ...
                        <div className="space-y-3">
                          {cat.featuredTreatments?.map((service, sIdx) => {
                              ...
                              <div key={sIdx} className="border border-brand-beige rounded-xl overflow-hidden">
                                ...
                                <div className="flex items-center justify-between p-3 bg-brand-white hover:bg-brand-beige/20 transition-colors">
                                  ...
                                  <div className="flex items-center gap-1 ml-4 shrink-0">
                                    ...
                                    <button onClick={() => handleMoveService(idx, sIdx, 'down')} disabled={sIdx === (cat.featuredTreatments.length - 1)} className="p-1 hover:text-brand-forest disabled:opacity-30"><ArrowDown className="w-3.5 h-3.5" /></button>
                                  ...
                                  <div className="p-4 border-t border-brand-beige bg-white space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                      <div>
                                        ...
                                        <input type="text" value={service.name} onChange={e => { const up = [...categories]; up[idx].featuredTreatments[sIdx].name = e.target.value; setCategories(up); }} className="w-full px-2.5 py-1.5 text-xs border rounded-md font-semibold text-brand-charcoal" />
                                      </div>
                                      <div className="grid grid-cols-2 gap-2">
                                        <div>
                                          <label className="block text-[10px] font-bold text-brand-charcoal/70 uppercase mb-1">Duration</label>
                                          <input type="text" value={service.duration || ''} onChange={e => { const up = [...categories]; up[idx].featuredTreatments[sIdx].duration = e.target.value; setCategories(up); }} className="w-full px-2.5 py-1.5 text-xs border rounded-md" placeholder="e.g. 60 Mins" />
                                        </div>
                                        <div>
                                          <label className="block text-[10px] font-bold text-brand-charcoal/70 uppercase mb-1">Tag (Optional)</label>
                                          <input type="text" value={service.tag || ''} onChange={e => { const up = [...categories]; up[idx].featuredTreatments[sIdx].tag = e.target.value; setCategories(up); }} className="w-full px-2.5 py-1.5 text-xs border rounded-md" placeholder="e.g. Most Popular" />
                                        </div>
                                      </div>
                                    </div>
                                    ...
                                    <div>
                                      ...
                                      <textarea rows={2} value={service.desc} onChange={e => { const up = [...categories]; up[idx].featuredTreatments[sIdx].desc = e.target.value; setCategories(up); }} className="w-full px-2.5 py-1.5 text-xs border rounded-md" />
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-brand-white/50 p-3 rounded-lg border border-brand-beige">
                                      <div>
                                        <label className="block text-[10px] font-bold text-brand-charcoal/70 uppercase mb-1">Suitable For</label>
                                        <input type="text" value={service.suitableFor || ''} onChange={e => { const up = [...categories]; up[idx].featuredTreatments[sIdx].suitableFor = e.target.value; setCategories(up); }} className="w-full px-2.5 py-1.5 text-xs border rounded-md" />
                                      </div>
                                      <div>
                                        <label className="block text-[10px] font-bold text-brand-charcoal/70 uppercase mb-1">Procedure</label>
                                        <input type="text" value={service.procedure || ''} onChange={e => { const up = [...categories]; up[idx].featuredTreatments[sIdx].procedure = e.target.value; setCategories(up); }} className="w-full px-2.5 py-1.5 text-xs border rounded-md" />
                                      </div>
                                      <div>
                                        <label className="block text-[10px] font-bold text-brand-charcoal/70 uppercase mb-1">Downtime</label>
                                        <input type="text" value={service.downtime || ''} onChange={e => { const up = [...categories]; up[idx].featuredTreatments[sIdx].downtime = e.target.value; setCategories(up); }} className="w-full px-2.5 py-1.5 text-xs border rounded-md" />
                                      </div>
                                      <div>
                                        <label className="block text-[10px] font-bold text-brand-charcoal/70 uppercase mb-1">Expected Results</label>
                                        <input type="text" value={service.expectedResults || ''} onChange={e => { const up = [...categories]; up[idx].featuredTreatments[sIdx].expectedResults = e.target.value; setCategories(up); }} className="w-full px-2.5 py-1.5 text-xs border rounded-md" />
                                      </div>
                                      <div>
                                        <label className="block text-[10px] font-bold text-brand-charcoal/70 uppercase mb-1">Price</label>
                                        <input type="text" value={service.price || ''} onChange={e => { const up = [...categories]; up[idx].featuredTreatments[sIdx].price = e.target.value; setCategories(up); }} className="w-full px-2.5 py-1.5 text-xs border rounded-md" />
                                      </div>
                                      <div>
                                        <label className="block text-[10px] font-bold text-brand-charcoal/70 uppercase mb-1">Key Benefits (Comma separated)</label>
                                        <input type="text" value={service.keyBenefits?.join(', ') || ''} onChange={e => { const up = [...categories]; up[idx].featuredTreatments[sIdx].keyBenefits = e.target.value.split(',').map(s=>s.trim()).filter(Boolean); setCategories(up); }} className="w-full px-2.5 py-1.5 text-xs border rounded-md" placeholder="Deep pore refinement, Instant glow" />
                                      </div>
                                    </div>
                                  </div>
                                )}
                          ...
                          {(!cat.featuredTreatments || cat.featuredTreatments.length === 0) && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={pageSettings.treatment_hero_badge} onChange={e => setPageSettings({...pageSettings, treatment_hero_badge: e.target.value})} className="w-full px-3.5 py-2 text-sm border rounded-xl" />
              </div>
              <div>
                <label className="block text-xs font-bold text-brand-charcoal uppercase mb-1">Main Headline</label>
                <input type="text" value={pageSettings.treatment_hero_title} onChange={e => setPageSettings({...pageSettings, treatment_hero_title: e.target.value})} className="w-full px-3.5 py-2 text-sm border rounded-xl font-serif" />
              </div>
              <div>
                <label className="block text-xs font-bold text-brand-charcoal uppercase mb-1">Subtitle</label>
                <textarea rows={3} value={pageSettings.treatment_hero_subtitle} onChange={e => setPageSettings({...pageSettings, treatment_hero_subtitle: e.target.value})} className="w-full px-3.5 py-2 text-sm border rounded-xl leading-relaxed" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-brand-charcoal uppercase mb-1">Primary Button Text</label>
                  <input type="text" value={pageSettings.treatment_cta_text} onChange={e => setPageSettings({...pageSettings, treatment_cta_text: e.target.value})} className="w-full px-3.5 py-2 text-sm border rounded-xl" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-brand-charcoal uppercase mb-1">Consultation Link</label>
                  <input type="text" value={pageSettings.treatment_consultation_link} onChange={e => setPageSettings({...pageSettings, treatment_consultation_link: e.target.value})} className="w-full px-3.5 py-2 text-sm border rounded-xl text-blue-600" />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="aspect-[4/3] rounded-xl overflow-hidden border mb-3">
                <img src={pageSettings.treatment_hero_image} alt="Hero" className="w-full h-full object-cover" />
              </div>
              <label className="w-full inline-flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold cursor-pointer bg-brand-forest text-white hover:bg-brand-charcoal transition-colors">
                <Upload className="w-3.5 h-3.5" /> <span>Upload New Hero Image</span>
                <input type="file" accept="image/*" className="hidden" onChange={(e) => handlePhotoUpload(e, undefined, 'treatment_hero_image')} />
              </label>
            </div>
          </div>
        </div>

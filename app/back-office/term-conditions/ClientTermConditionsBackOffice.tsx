...
export default function ClientTermConditionsBackOffice({ initialContent }: { initialContent?: Record<string, any> }) {
  const [isSaving, setIsSaving] = useState(false);
  const [saveStatus, setSaveStatus] = useState<{ type: 'success' | 'error'; message: string } | null>(null);

  const [formData, setFormData] = useState<Record<string, any>>({
    terms_hero_title: 'Terms & Conditions',
    terms_hero_subtitle: 'Terms governing your use of our website and clinic services',
    terms_effective_date: '1 September 2026',
    terms_contact_email: 'shazaestheticbali@gmail.com',
    terms_contact_phone: '+62 823-4217-6585',
    terms_contact_address: 'Seminyak, Bali, Indonesia',
    terms_last_updated: '1 September 2026',
    ...initialContent,
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveStatus({ type: 'success', message: 'Terms & Conditions content saved successfully!' });
  ...
  return (
    <div className="max-w-4xl mx-auto pb-24">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-serif text-brand-charcoal">Terms & Conditions Settings</h1>
          <p className="text-brand-charcoal/60 mt-2">Manage the content for the terms & conditions page.</p>
        </div>
        <button
          onClick={handleSave}
          disabled={isSaving}
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-brand-beige overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            <div>
              ...
              <input
                type="text"
                value={formData.terms_hero_title || ''}
                onChange={(e) => handleChange('terms_hero_title', e.target.value)}
            ...
            <div>
              ...
              <input
                type="text"
                value={formData.terms_hero_subtitle || ''}
                onChange={(e) => handleChange('terms_hero_subtitle', e.target.value)}
                className="w-full px-4 py-2 border border-brand-beige rounded-lg focus:outline-none focus:border-brand-charcoal focus:ring-1 focus:ring-brand-charcoal"
              />
            </div>
            <div>
              ...
              <input
                type="text"
                value={formData.terms_effective_date || ''}
                onChange={(e) => handleChange('terms_effective_date', e.target.value)}
                className="w-full px-4 py-2 border border-brand-beige rounded-lg focus:outline-none focus:border-brand-charcoal focus:ring-1 focus:ring-brand-charcoal"
              />
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-brand-beige overflow-hidden">
          <div className="p-6 border-b border-brand-beige">
            <h3 className="text-xl font-serif text-brand-charcoal">Contact Information (Section 14)</h3>
          </div>
          <div className="p-6 space-y-6">
            <div>
              ...
              <input
                type="email"
                value={formData.terms_contact_email || ''}
                onChange={(e) => handleChange('terms_contact_email', e.target.value)}
                className="w-full px-4 py-2 border border-brand-beige rounded-lg focus:outline-none focus:border-brand-charcoal focus:ring-1 focus:ring-brand-charcoal"
              />
            </div>
            <div>
              ...
              <input
                type="text"
                value={formData.terms_contact_phone || ''}
                onChange={(e) => handleChange('terms_contact_phone', e.target.value)}
                className="w-full px-4 py-2 border border-brand-beige rounded-lg focus:outline-none focus:border-brand-charcoal focus:ring-1 focus:ring-brand-charcoal"
              />
            </div>
            <div>
              ...
              <input
                type="text"
                value={formData.terms_contact_address || ''}
                onChange={(e) => handleChange('terms_contact_address', e.target.value)}
                className="w-full px-4 py-2 border border-brand-beige rounded-lg focus:outline-none focus:border-brand-charcoal focus:ring-1 focus:ring-brand-charcoal"
              />
            </div>
            <div>

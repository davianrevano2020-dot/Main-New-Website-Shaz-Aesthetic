...
export default function ShazHomepage() {
  const [siteContent, setSiteContent] = useState<any>({}); const [mounted, setMounted] = useState(false);
  ...
  useEffect(() => {
    setMounted(true); const fetchContent = async () => {
      try {
  ...
  if (siteContent.treatments_list) {
    try {
      const parsed = JSON.parse(siteContent.treatments_list);
  ...
  if (siteContent.packages_list) {
    try {
      const parsed = JSON.parse(siteContent.packages_list);
  ...
  if (siteContent.doctors_list) {
    try {
      const parsed = JSON.parse(siteContent.doctors_list);
  ...
  if (siteContent.reasons_list) {
    try {
      const parsed = JSON.parse(siteContent.reasons_list);
  ...
  if (siteContent.transformations_list) {
    try {
      const parsed = JSON.parse(siteContent.transformations_list);
  ...
  if (siteContent.reviews_list) {
    try {
      const parsed = JSON.parse(siteContent.reviews_list);
  ...
  if (siteContent.clinics_list) {
    try {
      const parsed = JSON.parse(siteContent.clinics_list);
